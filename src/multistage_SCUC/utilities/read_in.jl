...
"""
    function piecewise_linear_coefficients(...)

# Purpose
    Convert polynomial cost data into piecewise-linear segment coefficients for UC preprocessing.

# Arguments
    1. Polynomial coefficients and break-point values.

# Returns
    1. Piecewise-linear slope/intercept representation used in model construction.
"""
...
function scenario_tree_generation(; T::Int64 = 10, numRealization::Int64 = 3, indexSets::IndexSets = indexSets)
    ...
    for t in 2:T
        ...
        for n in 1:numRealization
            deviation = Dict{Int64, Float64}()
            for d in indexSets.D
                if 0 <= (t % 24) < 3   # peak time 
                    base_demand = 0.9
                elseif 4 <= (t % 24) < 9  # off-peak time
                    4 <= (t % 24) < 9   # peak time 
                    base_demand = 1.3
                else                    # non-peak time 
                    base_demand = 1.2
...
# Purpose
    Expand scenario-tree nodes into full scenario paths with probabilities.

# Arguments
    1. Current stage index, partial path, probability mass, and scenario tree data.

# Returns
    1. Collection of complete scenario paths and their associated metadata.
"""
