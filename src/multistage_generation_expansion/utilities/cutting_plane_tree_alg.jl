...
function cut_generation_lp(
    ...
    elseif param.normalization == :L1norm
        error("CPT CGLP normalization ':L1norm' is not implemented for generation expansion.")
    elseif param.normalization == :Facet
        error("CPT CGLP normalization ':Facet' is not implemented for generation expansion.")
    elseif param.normalization == :Weighted
        error("CPT CGLP normalization ':Weighted' is not implemented for generation expansion.")
    ...
    elseif param.normalization == :Uniform
        # variables
        @variable(model, λ[1:m, Leaf] ≥ 0);
        @variable(model, μ[1:n, Leaf] ≥ 0);
        @variable(model, ν[1:n, Leaf] ≥ 0);
        ...
        @variable(model, π_0);
        # normalization
        @constraint(model, sum(λ) + sum(γ) ≤ 1);
        # constraints
        @constraint(model, [τ in Leaf], π_' .== (λ[:, τ] - γ[:, τ])'A .+ μ[:, τ]' .- ν[:, τ]');
        # @constraint(model, [τ in Leaf], π_0 ≤ b'λ[:, τ] - b̄'γ[:, τ] + lbset[τ]'μ[:, τ] - ubset[τ]'ν[:, τ]);
        ex = Dict{Int, Any}();
...
"""
    function _gep_remove_nonanticipative_constraints!(...)

# Purpose
    Remove generation-expansion nonanticipativity constraints before CGLP/separation steps.

# Arguments
    1. Generation-expansion backward model.

# Returns
    1. In-place model update with nonanticipativity constraints removed.
"""
function _gep_remove_nonanticipative_constraints!(model::Model)::Nothing
    if :NonAnticipative ∈ keys(model.obj_dict)
        for idx in eachindex(model[:NonAnticipative])
            delete(model, model[:NonAnticipative][idx])
        end
...
"""
    function _gep_remove_surrogate_constraints!(...)

# Purpose
    Remove surrogate-Delta helper constraints from generation-expansion model.

# Arguments
    1. Generation-expansion backward model.

# Returns
    1. In-place cleanup of surrogate alpha and interpolation equalities.
"""
function _gep_remove_surrogate_constraints!(model::Model)::Nothing
...
"""
    function _gep_restore_anchor_nonanticipative!(...)

# Purpose
    Restore anchor nonanticipativity equalities in generation expansion after surrogate checks.

# Arguments
    1. Model and anchor state vector.
...
function _gep_restore_anchor_nonanticipative!(
    ...
    _gep_remove_surrogate_constraints!(model)
    _gep_remove_nonanticipative_constraints!(model)
    AddNonAnticipative!(model, state)
    return
end

"""
    function _gep_add_surrogate_edge_constraints!(...)

# Purpose
    Inject surrogate interpolation equalities for one generation-expansion copy-variable candidate.

# Arguments
    1. Model, anchor state, copy-variable list, and candidate index.

# Returns
    1. In-place model mutation for surrogate edge evaluation.
"""
function _gep_add_surrogate_edge_constraints!(
    model::Model,
    state::Vector{Float64},
...
"""
    function _gep_endpoint_objective!(...)

# Purpose
    Evaluate generation-expansion endpoint objective for surrogate Delta computation.

# Arguments
    1. Model and endpoint state vector.

# Returns
    1. Endpoint objective value or `nothing` when endpoint solve is infeasible.
"""
function _gep_endpoint_objective!(
    model::Model,
    endpoint_state::Vector{Float64},
)::Union{Nothing, Float64}
    _gep_restore_anchor_nonanticipative!(model, endpoint_state)
    optimize!(model)
    st = termination_status(model)
...
"""
    function _gep_evaluate_surrogate_candidate!(...)

# Purpose
    Evaluate one generation-expansion copy split candidate and compute surrogate Delta.

# Arguments
    1. Model, anchor state, copy-variable list, candidate index, matrix columns, and anchor objective.

# Returns
    1. Named tuple with feasibility, Delta, branch point, and fractional-point cache.
"""
function _gep_evaluate_surrogate_candidate!(
    model::Model,
    state::Vector{Float64},
    ...
    endpoint_value = _gep_endpoint_objective!(model, endpoint_state)
    if isnothing(endpoint_value)
        _gep_restore_anchor_nonanticipative!(model, state)
        return (
            feasible = false,
            delta = -Inf,
            branch_value = 0.5,
    ...
    _gep_add_surrogate_edge_constraints!(model, state, copy_vars, branch_position)
    optimize!(model)
    st = termination_status(model)
    if !(st == MOI.OPTIMAL || st == MOI.LOCALLY_SOLVED)
        _gep_restore_anchor_nonanticipative!(model, state)
...
function CPT_optimization!(
    ...
    while d < MDCiter
        ...
        if st == MOI.OPTIMAL || st == MOI.LOCALLY_SOLVED   ## local solution
            ...
            x̂_Lc = Dict{Any, Float64}()
            if use_copy_branching
                x̂_Lc = Dict(
                    tightenLPModel[:Lc][i] => value(tightenLPModel[:Lc][i]) for i in 1:param.binary_info.n
                )
            end
        else
        ...
        if use_copy_branching && enable_surrogate_copy_split && copy_split_strategy == :surrogate_delta
            ...
            copy_vars = [tightenLPModel[:Lc][i] for i in 1:param.binary_info.n]
            best_delta = -Inf
            best_position = 0
            best_branch_value = 0.5
            best_x_fractional = Dict{Int64, Float64}()
            for branch_position in eachindex(copy_vars)
                candidate = _gep_evaluate_surrogate_candidate!(
                    tightenLPModel,
                    state,
                    copy_vars,
                    branch_position,
        ...
        if surrogate_cut_added
            ...
            x̂_x = Dict(tightenLPModel[:x][i] => value.(tightenLPModel[:x][i]) for i in 1:param.binary_info.d)
            x̂_Lc = Dict{Any, Float64}()
            if use_copy_branching
                x̂_Lc = Dict(
                    tightenLPModel[:Lc][i] => value(tightenLPModel[:Lc][i]) for i in 1:param.binary_info.n
            ...
            all_integer_x = all(is_almost_integer.(values(x̂_x)))
            if all_integer_x
        ...
        primary_dev = Dict{Any, Float64}(); compute_deviation(x̂_x, primary_dev); 
        primary_value_map = Dict{Any, Float64}(var => Float64(val) for (var, val) in x̂_x)
        copy_dev = Dict{Any, Float64}()
        copy_value_map = Dict{Any, Float64}()
        if use_copy_branching
            compute_deviation(x̂_Lc, copy_dev);
            for (var, val) in x̂_Lc
