...
const SSLP_PROJECT_ROOT = normpath(joinpath(@__DIR__, "..", "..", ".."))
...
"""
    function _build_sslp_results_path(...)

# Purpose
    Build canonical SSLP result path from effective runtime parameters.

# Arguments
    1. SSLP runtime parameter bundle.

# Returns
    1. Absolute output file path.
"""
function _build_sslp_results_path(param::NamedTuple)::String
    results_root = hasproperty(param, :results_root) ? param.results_root : joinpath(SSLP_PROJECT_ROOT, "results")
    ...
    normalization = cut_selection ∈ [:DBC, :iDBC, :SPC, :FC] ? param.normalization : "na"
    ...
    dataset_id = join(
        [
            _kv_token("J", param.J),
            _kv_token("I", param.I),
            _kv_token("Om", param.omega_count),
        ],
        "__",
    )
    run_id = join(
    ...
    return joinpath(results_root, "sslp", dataset_id, "$run_id.jld2")
end
...
function save_info(
    ...
    cut_selection = param.cut_selection
    J, I, Ω = param.J, param.I, param.omega_count
    algorithm = param.algorithm
    normalization = param.normalization
    disjunction_iteration_limit = param.disjunction_iteration_limit
    D = cut_selection == :SPC ? 1 : (disjunction_iteration_limit >= 0 ? disjunction_iteration_limit : Inf)
    inherit_disjunctive_cuts = param.inherit_disjunctive_cuts
    enforce_binary_copies = param.enforce_binary_copies
    legacy_logger_paths = hasproperty(param, :legacy_logger_paths) ? param.legacy_logger_paths : false

    new_save_path = _build_sslp_results_path(param)
    run_meta = Dict(
        :project => "sslp",
        :algorithm => algorithm,
        :cut_selection => cut_selection,
        :dataset => Dict(:J => J, :I => I, :omega => Ω),
    ...
    base_path = joinpath(SSLP_PROJECT_ROOT, "src", "sslp", "logger", "numerical_results")
    prefix = "J$J-I$I-Ω$Ω"

    filename = if cut_selection ∈ [:DBC, :iDBC, :SPC]
        "$algorithm-$cut_selection-$inherit_disjunctive_cuts-$normalization-$disjunction_iteration_limit"
    elseif cut_selection == :BC
        "$cut_selection"
    elseif cut_selection == :FC
        "$cut_selection-$inherit_disjunctive_cuts-$enforce_binary_copies"
    elseif cut_selection ∈ [:SMC, :LC, :SBC]
        "$cut_selection-$enforce_binary_copies"
    else
        "$cut_selection-$enforce_binary_copies"
    end
...
# Purpose
    Assemble the canonical SSLP runtime parameter bundle.

# Arguments
    1. Canonical keyword parameters used by SSLP experiment configs.

# Returns
    1. Runtime parameter named tuple consumed by SSLP routines.
"""
function param_setup(;
    verbose::Bool = false,
    ...
    J::Int64 = 20,
    I::Int64 = 100,
    omega_count::Int64 = 50,
)::NamedTuple
    results_root_value = isnothing(results_root) ? joinpath(SSLP_PROJECT_ROOT, "results") : results_root
...
"""
    resolve_sslp_runtime_params(param::NamedTuple)::NamedTuple

# Purpose
    Return the canonical runtime-view used by SSLP algorithm components.
"""
function resolve_sslp_runtime_params(param::NamedTuple)::NamedTuple
    return (
        omega_count = param.omega_count,
