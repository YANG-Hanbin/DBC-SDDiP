...
function Fenchel_cut_optimization!(
    ...
    x̂ = nothing; st = nothing; fractional_point = nothing; Z = nothing; slope = nothing; 
    ...
    while d < MDCiter
        ...
        ## formulate CGCP and generate a disjunctive cut
        lp_model();  
...
function FuncInfo_LevelSetMethod(
    ...
    optimize!(model);
    runtime_param = resolve_sslp_runtime_params(param)
    if runtime_param.enforce_binary_copies
        F_solution = ( 
            F = JuMP.objective_value(model), 
            ∇F = round.(JuMP.value.(variables), digits = 2)
        );
    else
        F_solution = ( 
            F = JuMP.objective_value(model), 
...
function LevelSetMethod_FC_optimization!( 
    ...
    (μ, λ, threshold, nxt_bound, max_iter) = (
        ...
        level_set_method_param.MaxIter
    );
