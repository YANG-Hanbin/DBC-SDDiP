...
function FuncInfo_LevelSetMethod(
    ...
    if cut_selection ∈ [:PLC, :PLCwithoutConstraint, :PLBC]
        @objective(
            backward_model, 
            Min, 
            backward_model[:primal_objective_expression] + 
            x₀' * ( state .- backward_model[:z])
        );
        optimize!(backward_model);
        F_solution = ( 
            F = JuMP.objective_value(backward_model),                 
            ∇F = state .- JuMP.value.(backward_model[:z]) 
        );
...
function LevelSetMethod_optimization!( 
    ...
    (μ, λ, threshold, nxt_bound, MaxIter) = (
        param_LevelMethod.μ, 
        param_LevelMethod.λ, 
        param_LevelMethod.threshold, 
        param_LevelMethod.nxt_bound, 
        param_LevelMethod.MaxIter
    );
    ...
    @variable(oracle_model, x[i = 1:param.J]);
    ...
    @variable(next_model, x1[i = 1:param.J]);
    ...
    while true
        ...
        optimize!(next_model); st = termination_status(next_model);
        # @info "$st"
        if st == MOI.OPTIMAL || st == MOI.LOCALLY_SOLVED   ## local solution
            x_nxt = JuMP.value.(x1);
        ...
        if Δ ≤ param_LevelMethod.threshold || iter > MaxIter
            # @info "yes"
            return (cut_info = cut_info, iter = iter)
        end
...
# Purpose
    Construct per-backward-call level-set parameters using current anchor point and incumbent value.

# Arguments
    1. Backward model/anchor state/value plus global parameter bundles.

# Returns
    1. Prepared level-set parameter tuple and initial dual point.
"""
function setup_levelset_param(
